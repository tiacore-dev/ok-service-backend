...
- Вложения объекта: просматривать и скачивать могут `admin`, любой `manager`,
  любой `project-leader`, а также пользователь, указанный в `objects.manager`;
  изменять — только `admin` или пользователь, указанный в `objects.manager`.
- Вложения места: просматривать и скачивать могут `admin`, любой `manager`,
  любой `project-leader`, а также пользователь, указанный в `objects.manager`;
  изменять — только `admin` или пользователь, указанный в `objects.manager` для
  объекта, которому принадлежит место.
- Вложения сменного отчёта: `user` видит и скачивает вложения только собственной
  смены, в том числе после согласования; `manager` и `project-leader` видят и
  скачивают вложения доступной им смены. После согласования (`signed=true`)
  добавление и удаление вложений доступно только `admin`. До согласования
  `user` может удалить вложение своей смены; отчёт, связанный с отсутствием
  через `leave_id`, не изменяется.
