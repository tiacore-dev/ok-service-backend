...
- просмотр и списки доступны `admin`, `manager` и прорабу только для своих
  спецификаций; пользователю просмотр запрещён;
...
- создание, редактирование и удаление связей с работами также доступны только
  `admin` и `manager`.
- Приёмку со статусом `documents_signed` может редактировать только `admin`.
- Просмотр и скачивание вложений приёмки подчиняются тем же ролям и ownership.
...
Создание и редактирование приёмки запрещены, если её спецификация относится к
объекту в статусе `waiting` (`В ожидании`).
...
Загрузка и удаление доступны только `admin` и `manager`; просмотр и скачивание
доступны им и прорабу своей спецификации. Пользователь эти вложения не видит.
Валидация и хранение соответствуют `docs/attachments.md`.
...
При создании и редактировании связи количество проверяется отдельно для каждого
`work_id` в рамках спецификации приёмки. Доступное количество рассчитывается как
сумма `project_works.quantity` по проекту и работе минус сумма уже созданных
