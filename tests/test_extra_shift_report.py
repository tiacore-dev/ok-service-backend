...
def test_create_shift_report(client, jwt_token_user, db_session, seed_user, seed_project, seed_admin):
    ...
    report = db_session.query(ShiftReports).filter_by(
        shift_report_id=report_id).first()
...
def test_view_shift_report(client, jwt_token_user, jwt_token_admin, seed_shift_report, seed_admin):
    ...
    response = client.get(
        f"/shift_reports/{seed_shift_report['shift_report_id']}/view", headers=headers_user)
    assert response.status_code == 200

    # Админ может видеть всё
    response = client.get(
        f"/shift_reports/{seed_shift_report['shift_report_id']}/view", headers=headers_admin)
    assert response.status_code == 200

    # Если отчёт не существует
    response = client.get(
        f"/shift_reports/{uuid4()}/view", headers=headers_user)
    assert response.status_code == 404


# ✅ Тест редактирования (user не может редактировать чужие или подписанные)
def test_edit_shift_report(client, jwt_token_user, db_session, seed_shift_report, seed_project):
    headers_user = {"Authorization": f"Bearer {jwt_token_user}"}
    # headers_admin = {"Authorization": f"Bearer {jwt_token_admin}"}
    ...
    response = client.patch(
        f"/shift_reports/{seed_shift_report['shift_report_id']}/edit", json=data, headers=headers_user)
    assert response.status_code == 200

    # Если отчёт подписан → нельзя редактировать
    db_session.query(ShiftReports).filter_by(
        shift_report_id=seed_shift_report["shift_report_id"]).update({"signed": True})
    db_session.commit()
...
def test_soft_delete_shift_report(client, jwt_token_user, db_session, seed_shift_report):
    headers = {"Authorization": f"Bearer {jwt_token_user}"}

    response = client.patch(
        f"/shift_reports/{seed_shift_report['shift_report_id']}/delete/soft", headers=headers)
    assert response.status_code == 200
...
def test_hard_delete_shift_report(client, jwt_token_admin, db_session, seed_shift_report):
    headers = {"Authorization": f"Bearer {jwt_token_admin}"}

    response = client.delete(
        f"/shift_reports/{seed_shift_report['shift_report_id']}/delete/hard", headers=headers)
    assert response.status_code == 200
...
def test_get_all_shift_reports(client, jwt_token_user, jwt_token_admin, seed_shift_report):
    headers_user = {"Authorization": f"Bearer {jwt_token_user}"}
    headers_admin = {"Authorization": f"Bearer {jwt_token_admin}"}
