...
def works_manager(db_session):
    ...
    return WorksManager(session=db_session)  # type: ignore
...
def test_view_work(client, jwt_token, seed_work):
    ...
    """
    headers = {"Authorization": f"Bearer {jwt_token}"}
    response = client.get(f"/works/{str(seed_work['work_id'])}/view", headers=headers)
...
def test_soft_delete_work(client, jwt_token, seed_work, db_session):
    ...
    assert response.status_code == 200
    assert (
        response.json["msg"] == f"Work {seed_work['work_id']} soft deleted successfully"
    )
...
def test_hard_delete_work(client, jwt_token, seed_work, db_session):
    ...
    assert response.status_code == 200
    assert (
        response.json["msg"] == f"Work {seed_work['work_id']} hard deleted successfully"
    )
