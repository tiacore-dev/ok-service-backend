...
class UserAdd(Resource):
    ...
    def post(self):
        ...
        if current_user["role"] != "admin":
            logger.warning(
                "Несанкционированный запрос на добавление нового пользователя.",
    ...
    def get(self, user_id):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Неверный формат user_id: {user_id}",
                    extra={"login": current_user.get("login")},
                )
            ...
            logger.info(
                f"Пользователь user_id={user_id} найден успешно",
                extra={"login": current_user.get("login")},
            )
    ...
    def patch(self, user_id):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Неверный формат user_id при мягком удалении: {user_id}",
                    extra={"login": current_user.get("login")},
                )
                return {"msg": "Invalid user ID format"}, 400
            from app.database.managers.user_manager import UserManager
            ...
            if not updated:
                ...
                return {"msg": "User not found"}, 404
            logger.info(
                f"Пользователь user_id={user_id} мягко удален",
                extra={"login": current_user.get("login")},
            )
            return {
                "msg": f"User {user_id} soft deleted successfully",
                "user_id": user_id,
            }, 200
        except Exception as e:
            logger.error(
                f"Ошибка при мягком удалении пользователя user_id={user_id}: {e}",
                extra={"login": current_user.get("login")},
            )
...
class UserDeleteHard(Resource):
    ...
    def delete(self, user_id):
        ...
        if current_user["role"] != "admin":
            ...
            return {"msg": "Forbidden"}, 403
        logger.info(
            f"Запрос на окончательное (hard) удаление пользователя user_id={user_id}",
            extra={"login": current_user.get("login")},
        )
        try:
            # Преобразуем строку в UUID
            try:
                ...
                logger.warning(
                    f"Неверный формат user_id при hard удалении: {user_id}",
                    extra={"login": current_user.get("login")},
                )
                return {"msg": "Invalid user ID format"}, 400
            from app.database.managers.user_manager import UserManager
            ...
            if not deleted:
                ...
                return {"msg": "User not found"}, 404
            logger.info(
                f"Пользователь user_id={user_id} удален окончательно",
                extra={"login": current_user.get("login")},
            )
            return {
            ...
            logger.error(
                f"Ошибка при окончательном удалении пользователя user_id={user_id}: {
                    e
                }",
                extra={"login": current_user.get("login")},
            )
...
class UserEdit(Resource):
    ...
    def patch(self, user_id):
        ...
        try:
            ...
            logger.error(
                f"Ошибка валидации при редактировании пользователя: {err.messages}",
                extra={"login": current_user.get("login")},
            )
            return {"error": err.messages}, 400
        login = data.get("login")  # type: ignore
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Неверный формат user_id при редактировании: {user_id}",
                    extra={"login": current_user.get("login")},
                )
                return {"msg": "Invalid user ID format"}, 400
            from app.database.managers.user_manager import UserManager
            ...
            if password:
                logger.debug(
                    f"Обновление пароля пользователя user_id={user_id}...",
                    extra={"login": current_user.get("login")},
                )
            ...
            logger.error(
                f"Ошибка при редактировании пользователя user_id={user_id}: {e}",
                extra={"login": current_user.get("login")},
            )
...
class UserAll(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            logger.error(
                f"Ошибка валидации фильтров при получении пользователей: {
                    err.messages
                }",
                extra={"login": current_user.get("login")},
            )
        ...
        filters = {
            "login": args.get("login"),  # type: ignore
            "name": args.get("name"),  # type: ignore
            "role": args.get("role"),  # type: ignore
        ...
        try:
            ...
            users = db.get_all_filtered(offset, limit, sort_by, sort_order, **filters)  # type: ignore
            logger.info(
                f"Успешно получен список пользователей: количество={len(users)}",
                extra={"login": current_user.get("login")},
            )
