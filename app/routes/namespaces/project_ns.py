...
class ProjectStatsByProjectMaterials(Resource):
    ...
    def get(self, project_id):
        ...
        try:
            stats = db.get_project_stats_by_project_materials(project_id)
            if not stats:
                logger.warning(
                    f"No stats BY PROJECT MATERIALS found for project {project_id}",
                    extra={"login": current_user},
                )
                return {"msg": "Stats not found", "stats": {}}, 200
            logger.info(
                f"Stats BY PROJECT MATERIALS successfully fetched for project {
                    project_id
                }",
                extra={"login": current_user},
            )
            ...
            logger.error(
                f"Error getting stats BY PROJECT MATERIALS for project {project_id}: {
                    e
                }",
                extra={"login": current_user},
            )
