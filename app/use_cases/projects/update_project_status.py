...
class UpdateProjectStatusUseCase:
    ...
    def execute(
        ...
        if actor.role not in {"admin", "manager"}:
            raise ProjectForbiddenError("Only admin or manager can change project status")
        current = self.repository.get_project(project_id)
        if current is None or current.deleted:
            raise ProjectNotFoundError("Project not found")
        if (
            status != current.status
            and status not in ProjectStatus.neighbours(current.status)
        ):
            raise ValueError("Project status can only move to the adjacent status")
        updated = self.repository.update_project_status(project_id, current.status, status)
        if updated is None:
