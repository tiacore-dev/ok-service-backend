...
def _owned_project_ids(repository: ShiftReportRepository, actor: ShiftReportActor) -> set[UUID]:
    return {UUID(str(project_id)) for project_id in repository.get_project_ids_by_leader(actor.user_id)}
...
class ListShiftReportsUseCase:
    ...
    def execute(
        ...
        if actor.role == "user":
            ...
            elif any(project_id not in owned_project_ids for project_id in query.project):
                raise ShiftReportForbiddenError("Forbidden")
        total, reports = self.repository.list_shift_reports(**asdict(query))
        if actor.role == "user":
            reports = [report for report in reports if report.user == actor.user_id]
            total = len(reports)
