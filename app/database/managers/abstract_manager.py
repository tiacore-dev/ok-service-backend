...
class BaseDBManager(ABC):
    ...
    def get_by_id(self, record_id):
        ...
        try:
            logger.debug(
                f"Получение записи по ID: {record_id}", extra={"login": "database"}
            )
            with self.session_scope() as session:
                ...
                logger.warning(
                    f"Запись с ID {record_id} не найдена", extra={"login": "database"}
                )
                return None
        except Exception as e:
            logger.error(
                f"Ошибка при получении записи по ID: {e}", extra={"login": "database"}
            )
            raise
    ...
    def get_all(self, offset=0, limit=None):
        ...
        try:
            logger.debug(
                f"Получение всех записей, offset={offset}, limit={limit}",
                extra={"login": "database"},
            )
            with self.session_scope() as session:
            ...
            logger.error(
                f"Ошибка при получении всех записей: {e}", extra={"login": "database"}
            )
            raise
    ...
    def get_record_by_id(self, record_id):
        ...
        try:
            ...
            logger.error(
                "Error fetching record by ID: %s", e, extra={"login": "database"}
            )
            raise
    ...
    def update(self, record_id, **kwargs):
        ...
        try:
            ...
            if not filtered_kwargs:
                logger.warning(
                    "No valid fields provided for update: %s",
                    kwargs,
                    extra={"login": "database"},
                )
                return None
            ...
            with self.session_scope() as session:
                primary_key = inspect(self.model).primary_key[0].name  # type: ignore
                record = (
                    session.query(self.model)
            ...
            logger.error(
                "Error updating record with ID %s: %s",
                record_id,
                e,
                extra={"login": "database"},
            )
        ...
        try:
            ...
            with self.session_scope() as session:
                ...
                if record:
                    ...
                    logger.warning(
                        "Record not found for deletion: %s",
                        record_id,
                        extra={"login": "database"},
                    )
                    return None
            ...
            logger.error(
                "Cannot delete record %s due to integrity constraint: %s",
                record_id,
                e,
                extra={"login": "database"},
            )
            raise IntegrityError("Cannot delete record: dependent data exists.") from e  # type: ignore

        except Exception as e:
            logger.error(
                "Error deleting record with ID %s: %s",
                record_id,
                e,
                extra={"login": "database"},
            )
    ...
    def filter_by(self, **kwargs):
        ...
        try:
            ...
            logger.error(
                "Error filtering records by criteria %s: %s",
                kwargs,
                e,
                extra={"login": "database"},
            )
    ...
    def filter_one_by_dict(self, **kwargs):
        ...
        try:
            ...
            logger.error(
                "Error filtering one record by criteria %s: %s",
                kwargs,
                e,
                extra={"login": "database"},
            )
    ...
    def exists_by_id(self, record_id):
        ...
        try:
            ...
            logger.error(
                "Error checking existence by ID %s: %s",
                record_id,
                e,
                extra={"login": "database"},
            )
    ...
    def exists(self, **kwargs):
        ...
        try:
            logger.debug(
                f"Проверка существования записи: {kwargs}", extra={"login": "database"}
            )
            with self.session_scope() as session:
                exists = session.query(self.model).filter_by(**kwargs).count() > 0
                logger.info(
                    f"Существование записи: {exists}", extra={"login": "database"}
                )
                return exists
        except Exception as e:
            logger.error(
                f"Ошибка при проверке существования записи: {e}",
                extra={"login": "database"},
            )
            raise
    ...
    def get_all_filtered(
        ...
        with self.session_scope() as session:
            ...
            if offset:
                ...
                logger.debug(
                    f"Применяем смещение: offset = {offset}",
                    extra={"login": "database"},
                )
            if limit:
                query = query.limit(limit)
                logger.debug(
                    f"Применяем лимит: limit = {limit}", extra={"login": "database"}
                )
