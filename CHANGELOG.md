...
- Исправлена группировка фактической статистики `GET /project-leaders/get-stat`:
  месяцы формируются по календарю `Asia/Novosibirsk`, поэтому смена на 01.07
  больше не попадает в июнь. Общий контракт хранения Unix timestamp, API и
  миграции не изменялись.

- Исправлена регистрация вложенной Swagger-модели `WorkAcceptanceRelationBulkWork`,
  чтобы генерация `/swagger.json` не завершалась ошибкой после добавления bulk API.

- Добавлены bulk API для связей приёмки с работами: атомарное массовое создание
  по `acceptance_id` и списку `works`, а также массовое удаление по списку
  `relation_ids`. Повторяющиеся `work_id` сохраняются отдельными связями; API
  возвращает `ids`/`created_count` и `deleted_count`. Миграции и новые настройки
  не требуются.

- Для коллекционного `GET /project-leaders/get-stat` открыт доступ роли
  `project-leader`: прораб может просматривать статистику всех прорабов.
  Миграции, новые настройки и изменения формата API не требуются.

- Для коллекционного `GET /objects/get-stat` открыт доступ роли `project-leader`.
  Администратор и менеджер сохраняют доступ; миграции, новые настройки и
  изменения формата API не требуются.

- Добавлена проверка количества деталей приёмки по каждому `work_id` в рамках
  спецификации при создании и редактировании. При превышении количества
  спецификации запись не сохраняется; проверка и запись выполняются атомарно с
  блокировкой строк спецификации, а API возвращает `409` с кодом
  `WORK_ACCEPTANCE_QUANTITY_EXCEEDED` и подробными значениями лимита.
  Миграции и новые настройки не требуются.

- Изменён коллекционный `GET /project-leaders/get-stat`: при использовании
  `date_from` и `date_to` фактическая статистика теперь группируется по месяцам
  с ключами `YYYY-MM` и возвращается помесячно в общем итоге, по каждому
  прорабу и по каждому проекту. Новые параметры и миграции не добавлялись.

- Уточнены JWT-права по матрице для приёмок и вложений: добавлен ownership
  прораба для своих спецификаций, пользователю закрыт их просмотр, а приёмку
  со статусом `documents_signed` редактирует только администратор. CRUD и
  просмотр `project_works` этим изменением не менялись. API-key полномочия не
  изменялись.

- Добавлено автоматическое преобразование загружаемых PNG и JPEG/JPG-вложений в WebP
  с качеством 80. В имени, MIME-типе, размере и checksum сохраняемого вложения отражается
  уже преобразованный файл; перед конвертацией применяется EXIF Orientation; добавлена
  зависимость Pillow. API endpoint-ы не изменялись.
...
- Обновлён коллекционный `GET /project-leaders/get-stat`: добавлена отдельная
  фактическая агрегация по каждому проекту прораба, фильтры `date_from`,
  `date_to` и повторяемый `project_leader_ids`. В расчёт входят только детали
  подписанных неудалённых сменных отчётов; план и acceptance-агрегаты в новый
  расчёт не включаются. Существующие endpoint-ы конкретного прораба не
  изменены; миграции не требуются.
...
- Исправлена регистрация модели detail-ответа приёмки в Flask-RESTX Swagger;
  построение общей OpenAPI-схемы больше не падает с ошибкой `AcceptanceView not registered`.
...
- Добавлен подробный `GET /objects/{object_id}/get-stat-details` с группировкой
  статистики по проектам и `work_id`; сводный `get-stat` сохранён без изменений.

- Добавлены сводный и подробный методы статистики по прорабу:
  `GET /project-leaders/{project_leader_id}/get-stat` и
  `GET /project-leaders/{project_leader_id}/get-stat-details`. Статистика
  строится по неудалённым проектам, назначенным пользователю в поле
  `project_leader`; миграции не требуются.

- Исправлено умножение количества и суммы приемок при наличии нескольких строк
  `project_work` для одной работы; также усилена атомарная проверка ожидаемого
  статуса спецификации. Новых миграций и изменений API нет.

- Добавлена статистика приемок в `GET /projects/{project_id}/get-stat` и новый
  `GET /objects/{object_id}/get-stat` с общим итогом и группировкой по проектам.
  Для приемок возвращаются количество и сумма предъявленных и принятых работ;
  принятой считается приемка со статусом `documents_signed`. Добавлена
  документация `docs/object-statistics.md`; миграции и новые настройки не нужны.

- Добавлен идемпотентный bootstrap API-key permissions: при старте приложения в
  `permission_types` создаются только отсутствующие права. Канонический список
  расширен для всех текущих endpoint-ов с `api_key_or_jwt_required`; новые
  защищённые endpoint-ы необходимо добавлять в `app/utils/api_key_permissions.py`.
  Новых миграций и изменений API нет.

### Добавлено

- Исправлена атомарность проверки статуса спецификации при создании смены и условного обновления статуса; для `work_plan` уточнены Decimal-валидация и строковая сериализация `summ`, ограничены чтение и изменения ролями `admin` и `manager`, а ошибки больше не раскрывают внутренний текст исключений.

- Добавлен `GET /objects/statuses` с техническими значениями и русскими названиями статусов объектов; новых миграций не требуется.

- Для спецификаций добавлены enum-статусы `pending`, `in_progress`, `works_completed` и `closed`, отдельные `GET /projects/statuses` и `PATCH /projects/{project_id}/status`, ограничение создания смен статусом `in_progress`, а перевод в `closed` разрешён только при `signed=true` у всех `project_works`. Добавлена миграция `20260831120000_add_project_status.py`.

- Добавлена история изменения статусов приёмок: записи создаются при фактическом
  изменении статуса, доступны через `GET /acceptances/{id}/history` ролям `admin`
  и `manager`; добавлена миграция `20260828100000_acceptance_status_history`.

- Добавлена документация сущностей `acceptances` и `work_acceptance_relations`:
  описаны поля, статусы, CRUD API, права доступа и отложенные функции вложений
  и прогресса.

- В Redis-статистику работ добавлены плановая сметная сумма, фактическая сумма
  исполнителю и фактическая сумма по сметной цене; в ответы смен добавлено поле
  `shift_report_details_summ_by_estimate`. Учитываются только подписанные и не
  удалённые смены, строки без цены в сметную сумму не входят. Старый payload
  Redis автоматически пересчитывается; миграции БД не добавлялись.
...
- Добавлены сущности `acceptances` и `work_acceptance_relations` с CRUD API, статусами приёмки, связями с проектами и работами и миграцией базы данных. Изменены API и схема БД; вложения и расчёт прогресса пока не добавлялись.
## 2026-08-31

- Добавлена сущность `work_plan` с CRUD API, soft/hard delete и фильтрами по году, прорабу и общему плану.
- Добавлены ограничения: дата только первый день месяца, `summ >= 0`, один активный план на месяц для пользователя и компании.
- Добавлены миграция Alembic и unit-тесты; новых настроек и изменений существующих API нет.
## Изменения

- Добавлены проверки статусов: приёмки запрещены для объектов в ожидании,
  изменение статусов спецификаций для таких объектов запрещено, а закрытие
  спецификации требует подписанных работ и приёмок. Миграции и изменения API
  не добавлялись.
