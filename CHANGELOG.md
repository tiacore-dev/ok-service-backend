...
- Подготовлена Redis-инфраструктура для последующего кэширования агрегатов `project_works`: добавлены зависимость, `REDIS_URL`/`TEST_REDIS_URL`, ленивый клиент в `app.extensions` и документация будущего пересчёта без TTL. Текущий API и расчёт статистики пока не изменены.
