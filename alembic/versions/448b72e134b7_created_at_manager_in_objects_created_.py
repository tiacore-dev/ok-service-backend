...
def upgrade():
    ...
    op.add_column(
        "objects",
        sa.Column("manager", sa.UUID(), sa.ForeignKey("users.user_id"), nullable=False),
    )
    ...
    op.add_column("objects", sa.Column("created_by", sa.UUID(), nullable=False))

    # Добавляем ForeignKey (FK) на users.user_id
    op.create_foreign_key(
        "fk_objects_created_by",  # Название FK
        "objects",
        "users",  # Таблица-источник и таблица-цель
        ["created_by"],
        ["user_id"],  # Поля для связи
    ...
    op.add_column(
        "projects",
        sa.Column("night_shift_available", sa.Boolean, nullable=False, default=False),
    )
    op.add_column(
        "projects",
        sa.Column(
...
def downgrade():
    ...
    # Удаляем ForeignKey
    op.drop_constraint("fk_objects_created_by", "objects", type_="foreignkey")
